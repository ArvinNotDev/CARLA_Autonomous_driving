...
class LiveJunctionViewer:
    def __init__(
        self,
        host: str = "127.0.0.1",
        port: int = 2000,
        checkpoint_path: str = "junction_model_resnet18.pt",
        width: int = 1280,
        height: int = 720,
        fov: float = 90.0,
        camera_x: float = 1.4,
        ...
        # Keep your tuned camera values unchanged
    ...
    def _on_camera_image(self, image: carla.Image):
        ...
        bgr = array[:, :, :3]  # CARLA raw data is BGRA; drop alpha
        self.latest_bgr = bgr
    ...
    def draw_overlay(self, frame_bgr: np.ndarray):
        label_name = self.class_names.get(self.last_pred, str(self.last_pred))
        label_text = "JUNCTION" if self.last_pred == 1 else "NOT JUNCTION"
        ...
        cv2.putText(
            frame_bgr,
            f"Class: {label_name}  Conf: {self.last_conf:.3f}",
            (20, 80),
            cv2.FONT_HERSHEY_SIMPLEX,
            0.8,
            (255, 255, 255),
    ...
    def run(self):
        ...
        pygame.display.set_caption("CARLA Live Junction Classifier")
        ...
        try:
            while self.running:
                ...
                if self.latest_bgr is not None:
                    ...
                    # no rotation, no mirror. Just transpose for pygame surface layout.
...
def main():
    app = LiveJunctionViewer(
        host="127.0.0.1",
        port=2000,
        checkpoint_path="junction_model_resnet18.pt",
        width=1280,
        height=720,
        fov=90.0,
        camera_x=1.4,
