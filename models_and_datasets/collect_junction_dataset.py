...
#   Space     -> handbrake
#   E         -> save current frame with label 1 (junction)
#   Q         -> save current frame with label 0 (not junction)
#   ESC       -> quit
...
class CarlaDatasetCollector:
    def __init__(
        ...
        (self.out_dir / "0").mkdir(parents=True, exist_ok=True)
        (self.out_dir / "1").mkdir(parents=True, exist_ok=True)
    ...
    def spawn_camera(self):
        ...
        # Do not change this transform; it is user-tuned.
    ...
    def _on_camera_image(self, image: carla.Image):
        ...
        bgr = array[:, :, :3]  # BGRA -> BGR (drop alpha)
        self.latest_bgr = bgr
        ...
        with self.csv_path.open("a", newline="", encoding="utf-8") as f:
            ...
            writer.writerow(
                [
                    ...
                    f"{loc.z:.3f}",
                    f"{rot.yaw:.3f}",
                    int(self.reverse_mode),
                    ts,
                ]
            )

        print(
            f"[SAVED] {save_path} | label={label} | "
    ...
    def control_from_keyboard(self) -> carla.VehicleControl:
        ...
        # Forward motion: left key turns left, right key turns right.
        # Reverse motion: steering is inverted, which matches real vehicle behavior.
        if steer_left and not steer_right:
            control.steer = -STEER_VALUE if self.reverse_mode else STEER_VALUE
    ...
    def draw_status(self, surface, font):
        ...
        lines = [
            ...
            f"Reverse mode: {self.reverse_mode}",
            "WASD / arrows drive | R reverse | E save junction | Q save non-junction | ESC quit",
        ]
    ...
    def run(self):
        ...
        pygame.display.set_caption("CARLA Dataset Collector")
